...
> 维护说明：`当前可用链路/模块检测方式` 已废弃，不再作为工具、SOP、验收项或决策树节点出现。涉及链路/模块检查时，统一改为“当前可用的链路/模块检测方式、APP 报错、DA2、飞行日志、人工基础检查”。
...
## Issue 9：清理项目中已废弃的 当前可用链路/模块检测方式 工具引用

### 背景

当前可用链路/模块检测方式 已废弃。仓库里所有文档、决策树、案例、SOP 规划都不应再把它作为当前可用工具。
...
```text
1. 工具名过时：删除 当前可用链路/模块检测方式 名称，改为“当前可用的链路/模块检测方式”。
2. 流程依赖该工具：改成“人工基础检查 + APP 报错 + DA2/日志/可用测试工具”。
3. 不再新增 docs/sop/当前可用链路/模块检测方式_LINK_TEST.md。
4. 已有决策树节点不要继续要求用户使用当前可用链路/模块检测方式。
```
...
注意：当前可用链路/模块检测方式 已废弃，不要新增任何依赖该工具的 SOP、决策树节点、验收项或案例字段。涉及链路/模块检查时，改用当前可用的检查方式、APP 报错、DA2、飞行日志和人工基础检查。
