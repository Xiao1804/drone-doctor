...
↓
predictPath：预测一条排故路径
↓
DiagnosisGenerator：生成可能原因、步骤、工具、耗时、置信度
↓
返回：matchedTree + predictedPath + diagnosis + suggestedActions
```
...
| 工具使用流程 | 万用表、DA2、ET7KY13、电池助手等 |
| 维修后验证 | 功能测试、APP 测试、链路测试、飞行前检查 |
...
| 编号 | 问题 | 建议 |
|---|---|---|
| P2-1 | README 技术版本与实际依赖可能不一致 | 对齐 package.json |
| P2-2 | “准确率 92%”缺少公开评估依据 | 改成 MVP 验证表述 |
| P2-3 | 旧诊断接口要求 thinking 字段 | 改成证据链、不确定项、下一步检查 |
| P2-4 | 缺少自动化测试 | 补 freeUsage/user/unifiedDiagnosis/vector 测试 |
...
当前已有 MVP、诊断服务、决策树、反馈系统、PR 修复记录。建议做“工作流化整理”，而不是重写项目。

### 10.2 优先补救顺序

```text
1. 把 quick 模式改成“初步判断”，不要直接给最终定损结论
2. 新增 tree-flight-abnormal，专门解决“无法起飞/飞行异常”
3. 把 5 大决策树整理成可视化工作流节点表
4. 把用户反馈绑定到 diagnosisId / treeId / nodeId
5. 用真实维修案例反向修正知识库
6. 补自动化测试和验收脚本
```
